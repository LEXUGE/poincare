...
#show: simple.with(
  ...
  authors: ((name: "Candidate Number: 1076064", email: ""),),
  disp_content: false,
)
#import preamble: *
#import thm_vanilla: eg, thm_setup, def
#show: setup
#show figure: set block(breakable: true)
#show: thm_setup

= Introduction
Quantum Information experiments demand nanosecond-level timing precision and control over various hardware components, such as lasers and analog/digital converters (ADC). #link("https://github.com/m-labs/artiq/")[ARTIQ] is an open source control system, using FPGA and bare-metal firmware to deliver such precision.

Because of the low-level nature of FPGA, it is usually inconvenient to debug and inspect the components (e.g. lasers) controlled by ARTIQ in real-time. In the past, this requires users to hook on oscilloscopes manually for each input source. In this report, we propose an framework integrated into ARTIQ that allows user to monitor interesting data in real-time over Ethernet. We call this "streaming".

The streamer prototype we implemented in this project is capable of streaming the samples from "Sampler"#footnote[The name of the ADC is "Sampler".] ADC to ARTIQ Dashboard. It is designed to be compatible with messages in ARTIQ analyzer, hence through extension it can in principle stream a broad range of messages.

Finally, we demonstrate the working prototype by monitoring a simple Rabi-Flop single-qubit gate.
...
This project is done during June 17th to Aug 5th at the Oxford Ion Trap Quantum Computing group.
...
diagram(
	...
	mark-scale: 80%,
        ...
        node([_FPGA_], enclose: ((0,-0.5), (0,0.5)),
	...
	node((4,-0.5), [ARTIQ Dashboard], fill: colors.at(2)),
        node(align(bottom)[_Usual Computers_], enclose: ((2, 0), (4, -0.5), (4,0.5)),
          stroke: teal, fill: teal.lighten(90%),
          snap: -1,),
),
caption: [Basic setup]
...
ARTIQ uses certain type of FPGA boards (named "Kasli") to load experiment programs and dispatch commands. And the data we are interested in also resides on the board. In our setup, several Kasli will send the data over UDP to a central computer, where the `artiq_streamer` program will be running. Then the data is _published_ to various _subscribers_. Subscribers can choose what data to subscribe, and data will be displayed on the "ARTIQ Dashboard", where rest of the experiment aspects also reside in.
...
== Basic Setup
Data is only sent over when user _triggers_. After triggering, Kasli will send over data recorded in a "streaming window", where the `start` and `end` are relative to the time of trigger. This is illustrated in @fig:trigger.
#figure(
  diagram(
    edge-stroke: 1pt,
...
=== Obtain Data
The most important part of the project is to obtain (or _tap_) the data of interests. Usually, those data (e.g. ADC readings of the voltages of the photodiodes) are *not* accessible to the CPU of the Kasli, because processing those data must be done by hardware circuits of FPGA to ensure timing precision.

To resolve this, we need to design a protocol that allows FPGA (or _gateware_) to share data to CPU (where firmware is running), and use firmware to send data over network. Thus, we will focus on the design of how firmware and gateware work together.
...
For pseudo-code of the design, see @data-structure.
...
Each segment is sent in a single UDP packet to the publisher. The publisher will parse the packet and messages within it, and publishes each `message` as a ZMQ#footnote[A message communication protocol used by our publisher.] message, based on the RTIO channel#footnote[RTIO channel can be roughly understood as labels for different data sources.] contained within each message#footnote[`StopMessage` doesn't have a RTIO channel, and is sent in a ZMQ channel of its own: `STOP_CHANNEL`.].
...
= Results
== Function Generator
The setup is first tested tested with ADC connected to a function generator, and a Kasli connected to an ADC.
#figure(
  image("imgs/sinwave.png", width: 100%),
  caption: [
    Plot of streamed ADC samples with a connected function generator, viewed in dashboard
  ],
)<fig:function-generator-dashboard>
The latency of the displayed wave is correlated with the frequency of the function generated. This is expected as higher frequency of the function generated implies a higher frequency of message produced by ADC. And CPU's frequency is at an order of $10^(-3)$ slower than the FPGA's clock speed.

== Single-qubit Gate
Later, our setup is tested on a simple experiment, namely a single-qubit gate that performs Rabi-oscillation by applying laser on the trapped ion.

By applying an magnetic field $vb(B) = (0, B, 0)^T$ under the Hamiltonian $op(H) = mu_B vb(B) cdot vecop(S) $, the state $ket(psi)$ evolves as
$
mat(braket(0, psi); braket(1, psi)) = A_1 mat(cos Omega t; sin Omega t) + A_2 mat(sin Omega t; - cos Omega t)
$
where $Omega$ is the Rabi frequency. $ket(0) := ket(+z), ket(1) := ket(-z)$ and $A_1, A_2 in CC, |A_1|^2 + |A_2|^2 = 1$.

Thus the probabilities are oscillatory over time. If we control the interval the magnetic field is applied to the ion, then we can control the qubit's quantum state on the Bloch sphere.

And in the lab, we applied a sequence `X X x X X x x X` where `X` represents long pulse and `x` represents short pulse.
#figure(
  image("imgs/single-qubit-gate.png", width: 100%),
  caption: [
    Plot of photodiodes voltage measurement on laser pulses in a single qubit gate experiment, viewed in dashboard
  ],
)<fig:single-qubit-gate>

@fig:single-qubit-gate shows, in real-time, how laser sequence is applied to the trapped ion. Different pulse length of the laser "rotates" the quantum state of the qubit by different amount. And the measurement probabilities and the sequence programmed agrees with the pulse sequences from the waveform in @fig:single-qubit-gate.
...
#pagebreak()
= Appendix
In Rust-style psedo-code, the data-structure looks like
#figure(
```rust
let mut guard = 0;
...
The result looks like @fig:function-generator-dashboard, @fig:single-qubit-gate in dashboard.
